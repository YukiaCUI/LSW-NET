...
visualization_msgs::MarkerArray MapBuilderBridge::GetTrajectoryNodeList(geometry_msgs::PoseArray &pose_array,
 std::vector<cartographer::common::Time>& pose_timestamps) {
  ...
  for (const int trajectory_id : node_poses.trajectory_ids()) {
    ...
    // ROS_INFO("get header of pose array!");
    pose_array.header.stamp = ::ros::Time::now();
    pose_array.header.frame_id = node_options_.map_frame;
    ...
    for (const auto& node_id_data : node_poses.trajectory(trajectory_id)) {
      ...
      pose_array.poses.push_back(node_pose);
      const cartographer::common::Time& time = node_id_data.data.constant_pose_data.value().time;
      pose_timestamps.push_back(time);
...
visualization_msgs::MarkerArray MapBuilderBridge::GetConstraintList() {
  ...
  const auto constraints = map_builder_->pose_graph()->constraints();
  num_constraints = 0;
  double doubleAccumulatedInterLinearResidual =0;
  double doubleAccumulatedInterAngularResidual=0;
  double accumulatedInterLinearResidualTemp =0;
  double accumulatedInterAngularResidualTemp=0;
  // size_t residual0=0;
  // size_t residual1=0;
  // size_t residual2=0;
  // size_t residual3=0;
  std::ofstream outFile("/home/tju_dhj/catkin_ws/residual.txt",std::ios::trunc);
  outFile.close(); 
  outFile.open("/home/tju_dhj/catkin_ws/residual.txt", std::ios::app);
  for (const auto& constraint : constraints) {
    ...
    if (constraint.tag ==
      ...
      color_residual.r = 1.0;
       ...
       // 从 submap_poses 映射中根据子地图 ID 获取对应的 SubmapPose
        const auto& submap_pose_it = submap_poses.find(constraint.submap_id);
        // 从 trajectory_node_poses 映射中根据节点 ID 获取对应的 TrajectoryNodePose
        if (submap_pose_it == submap_poses.end()) {
            continue;}
        const auto& submap_pose = submap_pose_it->data.pose;
        const auto& node_pose_it = trajectory_node_poses.find(constraint.node_id);
        if (node_pose_it == trajectory_node_poses.end()) {
            continue;}
        const auto& node_pose = node_pose_it->data.global_pose;
        const Rigid3d constraint_pose = submap_pose * constraint.pose.zbar_ij;
        const Rigid3d error = node_pose.inverse() * constraint_pose;
        
        // // 累加残差
        // if(error.translation().norm()<10)
        // {
        //   residual0=residual0+1;
        // }
        // if(error.translation().norm()<5)
        // {
        //   residual0=residual1+1;
        // }
        // if(error.translation().norm()<3)
        // {
        //   residual0=residual2+1;
        // }
        // if(error.translation().norm()<1)
        // {
        //   residual0=residual3+1;
        // }
         // 打开一个文件流，用于写入
        num_constraints =num_constraints+1;
        outFile<<"位移残差:"<<error.translation().norm()<<"角度残差："<<std::abs(Eigen::AngleAxisd(error.rotation()).angle()) * (180.0 / M_PI)<<std::endl;

        accumulatedInterLinearResidualTemp += error.translation().norm();
        accumulatedInterAngularResidualTemp += std::abs(Eigen::AngleAxisd(error.rotation()).angle()) * (180.0 / M_PI);
        doubleAccumulatedInterLinearResidual+= error.translation().norm() * error.translation().norm();
        doubleAccumulatedInterAngularResidual+= std::pow(std::abs(Eigen::AngleAxisd(error.rotation()).angle()) * (180.0 / M_PI), 2);
  ...
  // 关闭文件流
  outFile.close();
 accumulatedInterLinearResidual=accumulatedInterLinearResidualTemp;
 accumulatedInterAngularResidual=accumulatedInterAngularResidualTemp;
 rmsLinearError =std::sqrt(doubleAccumulatedInterLinearResidual / num_constraints);
 rmsAngularError = std::sqrt(doubleAccumulatedInterAngularResidual / num_constraints);
//  residual_composition0=residual0;
//  residual_composition1=residual1;
//  residual_composition2=residual2;
//  residual_composition3=residual3;
