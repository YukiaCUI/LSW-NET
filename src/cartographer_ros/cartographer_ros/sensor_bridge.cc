...
void SensorBridge::HandleLaserScanMessage(
  ...
  //       std::cout<<point_cloud.intensities[i];
  // }
...
void SensorBridge::HandleLaserScan(
 ...
 // 创建一个 vector 来存储强度信息
  ...
  intensities.reserve(points.points.size());
////////////////
  ...
  for (int i = 0; i != num_subdivisions_per_laser_scan_; ++i) {
    ...
    ///////// 将强度信息存储到 intensities 向量中
   for (auto intensity : points.intensities) {
    intensity = std::max(0.1f, std::min(intensity, 0.5f));
    ...
    HandleRangefinder(sensor_id, subdivision_time, frame_id, subdivision, intensities);
  }
}

void SensorBridge::HandleRangefinder(
    const std::string& sensor_id, const carto::common::Time time,
    const std::string& frame_id, const carto::sensor::TimedPointCloud& ranges,const std::vector<float> intensities) {
