...
Node::Node(
  ...
  pose_array_publisher_ = node_handle_.advertise<geometry_msgs::PoseArray>("/gt_pose_array", 1);
...
bool Node::HandleTrajectoryQuery(
  ...
  // std::string txt_filename = "/home/tju_dhj/catkin_ws/test/trajectory/dianxin6_basic.txt";
...
void Node::PublishTrajectoryNodeList(
  ...
  if (trajectory_node_list_publisher_.getNumSubscribers() > 0) {
    ...
    trajectory_node_list_publisher_.publish(
        map_builder_bridge_.GetTrajectoryNodeList(pose_array, pose_timestamps));
...
const cartographer_ros::MapBuilderBridge& Node::getMapBuilderBridge() const {
    return map_builder_bridge_;
}

void Node::savePoseArray(std::string out_filename)
{
  constexpr int kUtsTicksPerSecond = 10000000;
  std::ofstream f_out;
  	f_out.open(out_filename.c_str());
  	f_out << std::fixed;	// 设置浮点数输出为固定小数格式
  int size = pose_array.poses.size();
	for(int i=0; i<size; i++) {
    // 转换时间戳
    double time = cartographer::common::ToSeconds(pose_timestamps[i] - cartographer::common::FromUniversal(cartographer::common::kUtsEpochOffsetFromUnixEpochInSeconds * kUtsTicksPerSecond));
    
		// set rotation matrixs and translation vector values.

	    f_out << std::setprecision(9) 
        << time << " "
        << pose_array.poses[i].position.x << " "
        << pose_array.poses[i].position.y << " "
        << tf2::getYaw(pose_array.poses[i].orientation) << std::endl;
