...
def train(data_path, batch_size):
    ...
    encoder_params_path = "/share/home/tj90055/dhj/Self_Feature_LO/src/point_cloud_processing/model/LSencoder_new/encoder_params.pth"
    ...
    # encoder_only_model.pe_sine1.load_state_dict(encoder_state_dict["pe_sine1"])
    ...
    # encoder_only_model.pe_sine2.load_state_dict(encoder_state_dict["pe_sine2"])
    ...
    # encoder_only_model.pe_sine3.load_state_dict(encoder_state_dict["pe_sine3"])
    encoder_only_model.t_encoder3.load_state_dict(encoder_state_dict["t_encoder3"])

    # encoder_only_model.pe_sineT.load_state_dict(encoder_state_dict["pe_sineT"])
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}")):
            ...
            curvatures = batch[...,1]
            curvatures = curvatures[:,2,:].squeeze(-1).float()
            t_mse = batch[...,2]
            t_mse = t_mse[:,2,:].squeeze(-1).float()
            B, T, N, _ = inputs.shape

            # input使用一阶段编码器进行encoder
            # print(f"inputs dtype: {inputs.dtype}")  # 检查数据类型
            inputs = inputs.float()
            ...
            features, weights = model(x_encoder)
            ...
            loss = multi_loss(features, weights, curvatures, t_mse).float()
...
if __name__ == "__main__":
    data_paths = "/share/home/tj90055/dhj/Self_Feature_LO/src/point_cloud_processing/src/LSWNet/data/pre_train_data_new.npy"
    batch_size = 64
