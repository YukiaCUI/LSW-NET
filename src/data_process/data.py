...
class ScanData(object):
    def __init__(self,data_file, data_length, batch_size, raw_length, sample_times, inf=35):
        ...
        for fname in data_file:
            with open("/share/home/tj90055/dhj/Self_Feature_LO/src/point_cloud_processing/data/" + fname, "r", encoding="utf8") as f:
    ...
    def get_next_batch(self):
        ...
        negative_index1 = random.sample(self.keys, self.batch_size)  
        negative_index2 = random.sample(self.keys, self.batch_size)  
        negative_isempty = np.random.randint(0, 2, (self.batch_size,))                      # 用于判断负采样数据有无障碍物
        negative_isempty1 = np.random.randint(0, 2, (self.batch_size,)) 
        negative_isempty2 = np.random.randint(0, 2, (self.batch_size,)) 
        # TODO: try
        ...
        for i in range(self.batch_size):
            ...
            if negative_isempty1[i] == 1:
                yn1.append(self.data[negative_index1[i]]["empty"][pos][left:right])
            else:
                yn1.append(self.data[negative_index1[i]]["full"][pos][left:right])
            if negative_isempty2[i] == 1:
                yn2.append(self.data[negative_index2[i]]["empty"][pos][left:right])
            else:
                yn2.append(self.data[negative_index2[i]]["full"][pos][left:right])
            if i % 2 == 0:
                pos += 1
            else:
        ...
        yn1 = self.X - np.array(yn1)
        yn2 = self.X - np.array(yn2)
